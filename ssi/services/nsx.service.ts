...
export const getVMTagsGroupsAndMembers = async (
  ...
  const scopedVirtualMachines: VMwareNSXVirtualMachine[] =
    ...
    >(
      ...
      (error: HTTPError) => {
        logger.error(
          `nsx-firewall-ssi: Could not retrieve VMs from NSX ${nsx.getHostname()} due to ${error.message}`,
          {
            component: "nsx.service",
            method: "getVMTagsGroupsAndMembers",
...
const getGlobalManagerEnforcementPointGroupMembers = async (
  ...
  for (const enforcementPoint of enforcementPoints) {
    const ipAddressesFromEP = await nsx.groups.getGroupMemberIPAddresses(
      ...
      logger.error(
        `nsx-firewall-ssi: Could not retrieve enforcement point group members from NSX ${nsx.getHostname()} for group ${group.id} due to ${error.message}`,
        {
          component: "nsx.service",
          method: "getGlobalManagerEnforcementPointGroupMembers",
          error: isDevMode() ? error : error.message,
        },
      );
      throw error;
